...
import com.training.springboot.exception.InsuffecientSalaryException;
...
//    }
    @GetMapping("/employees/{id}")
    private Employee getEmpById(@PathVariable("id") int id){
        if(employeeService.averageSalary() < employeeService.getEmployeeById(id).getSalary()){
            throw new InsuffecientSalaryException("Insuffecient Salary Exception-" + id);
        }
        return employeeService.getEmployeeById(id);
    }
